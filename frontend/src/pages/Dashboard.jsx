...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (selectedSource) {
      ...
      if (!socket) {
        const newSocket = io("http://localhost:5000", { transports: ["websocket"] });
        setSocket(newSocket); // ✅ Fix: Set socket before adding listeners

        newSocket.on("connect", () => console.log("✅ WebSocket Connected"));

        newSocket.on("update_dashboard", (data) => {
  ...
  }, [location.search, socket]); // ✅ Fix: Depend on `socket` to prevent multiple connections
